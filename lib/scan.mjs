...
// Where a project entry point (CLAUDE.md, CLAUDE.local.md, AGENTS.md) points: Claude @imports (its `includes`) and plain Markdown links,
// kept apart because they are different things (Claude Code expands an import; a link is only text). Each target is resolved from the
// referring file and gets one status: present (inside the project; size in bytes when it is a file), missing (inside the project,
// nothing there) or outside (the path, or a symlink on the way, leaves the project, so it is not stat'd as project content).
// Only status and size are kept: a target's contents never enter the scan.
async function resolveRef(ref, from, projectPath, root) {
  const target = ref.startsWith('~/') ? path.join(HOME, ref.slice(2)) : path.resolve(path.dirname(from), ref);
  const outside = { ref, path: target, status: 'outside', size: null };
  if (!coversPath(projectPath, target)) return outside;
  const real = await fs.realpath(target).catch(() => null);
  if (real) { if (!coversPath(root, real)) return outside; const s = await fs.stat(real).catch(() => null); return { ref, path: target, status: 'present', size: s?.isFile() ? s.size : null }; }
  // Nothing there. If the directory it would be in already leaves the project, it is outside, whether or not a file exists out there.
  const dir = await fs.realpath(path.dirname(target)).catch(() => null);
  return dir && !coversPath(root, dir) ? outside : { ref, path: target, status: 'missing', size: null };
}
// Relative links to local files, e.g. [harness.md](harness.md). Fenced and inline code and images are skipped, as are URLs with a scheme
// (http:, https:, mailto:, …), protocol-relative and #anchor-only links. A #fragment or ?query is dropped from the target.
function localLinks(text) {
  const prose = text.replace(/^(```|~~~)[^\n]*\n[\s\S]*?^\1[^\n]*$/gm, '').replace(/`[^`\n]*`/g, '');
  const refs = [];
  for (const m of prose.matchAll(/(?<!!)\[[^\]\n]*\]\(\s*<?([^)\s>]+)>?(?:\s+"[^"]*")?\s*\)/g)) {
    const raw = m[1], target = raw.replace(/[#?].*$/, '');
    if (!target || /^[a-z][a-z0-9+.-]*:/i.test(raw) || raw.startsWith('//')) continue;
    try { refs.push(decodeURIComponent(target)); } catch { refs.push(target); }
  }
  return [...new Set(refs)].slice(0, 50);
}
async function resolveRefs(file, text, includes, projectPath, root) {
  return {
    includesResolved: await Promise.all(includes.map((r) => resolveRef(r, file, projectPath, root))),
    linksResolved: await Promise.all(localLinks(text).map((r) => resolveRef(r, file, projectPath, root))),
  };
}
...
async function scanProject(p, ctx) {
  ...
  const instructions = [];
  const entry = async (provider, scope, file, content, includes) => ({ provider, scope, path: file, content, includes, ...(await resolveRefs(file, content, includes, p, root)) });
  if (claudeMd != null) instructions.push(await entry('claude', 'PROJECT', path.join(p, 'CLAUDE.md'), claudeMd, [...claudeMd.matchAll(/^@(\S+)/gm)].map((m) => m[1])));
  if (claudeLocalMd != null) instructions.push(await entry('claude', 'PROJECT-LOCAL', path.join(p, 'CLAUDE.local.md'), claudeLocalMd, []));
  if (agentsMd != null) instructions.push(await entry('codex', 'PROJECT', path.join(p, 'AGENTS.md'), agentsMd, []));
